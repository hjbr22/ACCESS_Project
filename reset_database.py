...
expanse_parsed = get_modules_and_versions(modules[5])
faster_parsed = get_modules_and_versions(modules[6])
jetstream_parsed = get_modules_and_versions(modules[7])
stampede_parsed = get_modules_and_versions(modules[8])
...
expanse_mod = organize_modules(expanse_parsed, "Expanse")
...
RpSoftware.insert_many(expanse_mod).on_conflict_replace().execute()
